 ...
 * 
 * Course ID support added by: Gunnar Dalton
 * 
...
const CanvasTokenManager = () => {
    ...
    const [courseID, setCourseID] = useState(""); // State for storing the course ID
    ...
    /**
     * Effect Hook: Load the stored course ID when the component mounts.
     * Fetches the course ID from `chrome.storage.local` and updates the state.
     */
    useEffect(() => {
    ...
    /**
     * Saves the entered course ID to Chrome storage.
     * Validates that the input is not empty before storing.
     */
    const saveCourseID = () => {
        if (!courseID.trim()) {
