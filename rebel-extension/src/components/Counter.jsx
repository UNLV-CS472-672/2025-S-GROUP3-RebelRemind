...
function Counter() {
  ...
  useEffect(() => {
    if (count > 5 && !hasFetched.current) {
      ...
      new Promise((resolve) => {
        // Check preferences to ensure Canvas integration is enabled.
        chrome.runtime.sendMessage({ type: "GET_PREFERENCES" }, (response) => {
          if (response && response.preferences) {
            const canvasIntegrationPreference =
            ...
            // No preferences stored in storage.
            setCanvasIntegration(false);
        ...
        if (canvasIntegrationPreference) {
          // Go ahead with GET_SCHEDULE since Canvas integration is enabled.
    ...
    <div>
      ...
      <p>
        {isFetchingSchedule
          ...
          ? canvasIntegration
            ? `${schedule}`
            : "Canvas Integration is disabled, cannot display schedule!"
